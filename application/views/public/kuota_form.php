	...
	<div class="modal-body">
		...
		<div class="form-group form-group-sm" >
			...
			<div class="row">
				...
				Bukti transfer harap disimpan dalam format jpg,jpeg,png , selain file tersebut kami tolak . Terimakasih <br>
				<label>File Input: <input type="file" name="file" id="demo1" name="demo1" /></label>
				<div id="uploads"></div>
				</div>
			</div>
...
$(document).ready(function() {
	...
	function applyAjaxFileUpload(element) {
		$(element).AjaxFileUpload({
			...
			onComplete: function(filename, response) {
				...
				$("#uploads").text(response.kode_transfer);
	...
	$("#kuota_form").validate({
	    rules: {
				nominal_transfer:{required:true}
				},
  		messages :{
	 		nominal_transfer:{required:"Upload file bukti transfer"}
      	},
	 	submitHandler: function(form) {
	        var harga=$('#nominal_transfer').val();
	  	    var konversidownload=$('#kuota_download').val();
	        var kodeuploads=$('#uploads').text();
	        if (kodeuploads =='' || kodeuploads=='anda belum mengupload bukti bayar')
	        {
	        	$('#uploads').text('anda belum mengupload bukti bayar');
	        }else{
	        $('#uploads').load(base_url+'index.php/user_public/save_kuota/',{"harga":harga,"konversidownload":konversidownload,"kodeuploads":kodeuploads},function (data){tarif();$('#myModalDetail').modal('hide');});
	        // alert (kodeuploads);
