...
<script>
function get_xls_laporan_anggota_kota_link(tanggal_start,tanggal_end,group_user,group_user_text){
	//alert(tanggal_end);
	//alert('hai');
	var form = document.createElement("form");
	form.setAttribute("method", "post");
	form.setAttribute("action", base_url+"index.php/admin/get_xls_laporan_anggota_kota");
...
<div>
	<a href="#" onclick="get_xls_laporan_anggota_kota_link('<?php echo $range_tanggal[0];?>','<?php echo $range_tanggal[1];?>','<?php echo $group_user;?>','<?php echo $group_user_text;?>')" >
	...
	<table class="table table-bordered table-hover table-striped">
		...
		foreach ($q_kuota_transfer->result() as $row_kuota_transfer){
			...
			echo '<tr>';
		        ...
		        echo '<td>'.$row_kuota_transfer->kabupaten_kota.'</td>';
