...
// do not change
$MY_URI = "http://localhost/oai2/oai2.php";

// do not change
$protocolVersion      = '2.0';

// *********** REKOMENDASI UNTUK DIUBAH - KONFIGURASI LOKAL ********************
include ('oaidp-config-server.php');
...
function getListSets() {
  global $errors,$db;
  $sets = array();
  $setquery = "select name_cat as set_name from category";
  $setresult = $db->query($setquery);
	if (DB::isError($setresult)) {
  	if ($SHOW_QUERY_ERROR) {
  		echo __FILE__.','.__LINE."<br />";
  		echo "Query: $query<br />\n";
  		die($db->errorNative());
    } else {
			$errors .= oai_error('cannot build sets', '', $identifier); 
		}
	} elseif (!$setresult->numRows()) {
		$errors .= oai_error('cannot build sets', '', $identifier); 
	}
  if ($errors != '') {
  	oai_exit();
  }
  $num_rows = $setresult->numRows();
  ...
  while ($record = $setresult->fetchRow()) {
    $set = $record['set_name'];
    $setname = $set;
    if (strlen($record['sub_set_name']) > 0) {
      $set .= ":" . $record['sub_set_name'];
      $setname .= " - " . $record['sub_set_name'];
    }
    array_push($sets, array('setSpec'=>$set, 'setName'=>$setname));
  }
  return $sets;
}
...
function getAuthors($id) {
  global $errors,$db;
  $sets = "";
  $setquery = "SELECT group_concat(CAST(m.authorname AS CHAR) separator '; ')   as authors
                FROM author_baru a 
                LEFT JOIN `masterauthor_baru` m ON a.author_id = m.author_id 
                WHERE a.id_jurnal = '" . $id ."' 
                GROUP BY a.author_id";
				//"select b.authorname from author_baru a left join masterauthor_baru b on a.author_id = b.author_id where a.id_jurnal = '" . $id ."'";
  $setresult = $db->query($setquery);
	if (DB::isError($setresult)) {
  	if ($SHOW_QUERY_ERROR) {
  		echo __FILE__.','.__LINE."<br />";
  		...
  		die($db->errorNative());
			...
			$errors .= oai_error('cannot find authors', '', $identifier); 
		}
	} elseif (!$setresult->numRows()) {
 		echo "Selesai";
	}
...
{
	global $SQL,$ISJD,$earliestDatestamp,$db;
  $query = "select 
              a.id_jurnal as ".$SQL['id_column'].",
              '" . $ISJD['provider'] . "' as provider,
              concat('".$ISJD['baseURL']."', a.id_jurnal) as url,
              IFNULL(a.tanggal,'".$earliestDatestamp."') as enterdate,
              a.id_jurnal as ".$SQL['identifier'].",
              b.name_cat as oai_set,
              IFNULL(a.tanggal,'".$earliestDatestamp."') as datestamp,
              'false' as deleted,
              a.title as dc_title,
              b.name_cat as dc_subject,
              a.abstract as dc_description,
              '' as dc_creator,
              '' as dc_contributor,
              a.institution as dc_publisher,
              a.tanggal as dc_date,
              'text' as dc_type,
              'pdf' as dc_format,
              concat('".$ISJD['baseURL']."', a.id_jurnal) as dc_identifier,
              '' as dc_source,
              '' as dc_language,
              '' as dc_relation,
              '' as dc_coverage,
              a.institution as dc_rights
            from jurnal as a
            left join category b on a.id_category = b.id_category 
            WHERE a.tanggal >= '".$earliestDatestamp."' GROUP BY a.id_jurnal";
	return $query;
}
...
function idQuery ($id = '')
{
	...
	if ($SQL['set'] != '') {
     $query = "select 
            ...
            from jurnal as a
            left join category b on a.id_category = b.id_category 
            WHERE a.tanggal >= '".$earliestDatestamp."' AND ";
	} else {
     $query = "select 
              concat('oai:".$ISJD['provider'].":".$ISJD['appname'].":',a.journal_id) as ".$SQL['identifier'].",
              IFNULL(a.tanggal,'".$earliestDatestamp."') as datestamp
            from jurnal as a
            WHERE a.tanggal >= '".$earliestDatestamp."' AND ";
 	}
	...
	if ($id == '') {
		$query .= "a." . $SQL['id_column'].' = a.'.$SQL['id_column'];
	}
	else {
		$query .= "a." . $SQL['id_column']." ='$id'";
	}
...
function setQuery($set)
{
	...
	return " and b.name_cat LIKE '%$set%'";
}
